...
import { BasicOrder, Order, OrderWithDetails } from "../interface/order";
...
export const update = (id: Number, order: Order, callback: Function) => {
    const queryString = `UPDATE ProductOrder SET product_id=?, customer_id=?, product_quantity=? WHERE id=?`;

    db.query(
        queryString,
        [order.product.id, order.customer.id, order.productQuantity, id],
        (err, result) => {
            if (err) { callback(err) }
...
export const destroy = (id: Number, callback: Function) => {
    const queryString = `DELETE FROM productorder WHERE id = ?;`;
